...
public class CompetitionLiveJsonUtil {
	...
	private final String COMPETITION_DATA_TYPE   = "application/vnd.playup.sport.competition+json";
	private final String SPORT_DATA_TYPE   		 = "application/vnd.playup.sport.sports+json";

	private boolean inTransaction = false;
	public CompetitionLiveJsonUtil ( String  str, boolean inTransaction ) {
...
private void parseData ( String  str) {
	...
	try {
		...
		String vCompetitionLiveId = jsonObject.getString(UID_KEY);

		
		JSONArray items = jsonObject.getJSONArray(ITEMS_KEY);
		...
		String vContestHref = jsonObj.optString( HREF_LINK_KEY );
		
	

		dbUtil.setHeader( vContestHref,vContestUrl, jsonObj.getString( TYPE_KEY ), false  );
		...
		String competitionName = jsonObj.getString(COMPETITION_NAME_KEY);
		...
		String assoiciatedContestUrl = "",associatedContestId = "",assoiciatedContestHrefUrl = "";

		if( jsonObj.has(ASSOCIATED_CONTESTS_KEY) ) {
			
			JSONObject associatedJsonObj = jsonObj.getJSONObject(ASSOCIATED_CONTESTS_KEY);
			if(associatedJsonObj.getString(TYPE_KEY).equalsIgnoreCase(Types.COLLECTIONS_DATA_TYPE))
			{
				assoiciatedContestUrl = associatedJsonObj.optString(SELF_KEY);

				assoiciatedContestHrefUrl = associatedJsonObj.optString(HREF_LINK_KEY);

				associatedContestId =  associatedJsonObj.optString(UID_KEY);
				dbUtil.setHeader(assoiciatedContestHrefUrl,assoiciatedContestUrl, associatedJsonObj.getString(TYPE_KEY), false);
...
//			new Util().getAssociatedContests(assoiciatedMatchUrl);
		...
		for ( int k = 0, ancestor_jArr_len = ancestor_jArr.length(); k < ancestor_jArr_len; k++ ) {
			...
			if ( ancestor_jArr_jObj.getString( TYPE_KEY).equalsIgnoreCase( ROUND_DATA_TYPE ) ||
					ancestor_jArr_jObj.getString( TYPE_KEY).equalsIgnoreCase( Types.GROUPINGS_DATA_TYPE )) {
			...
			/*	dbUtil.setRoundData(vRoundId, values );*/
				dbUtil.setRoundData(vRoundId, values );
			} else if ( ancestor_jArr_jObj.getString( TYPE_KEY).equalsIgnoreCase( COMPETITION_DATA_TYPE ) ) {
			...
			} else if ( ancestor_jArr_jObj.getString( TYPE_KEY).equalsIgnoreCase( SPORT_DATA_TYPE ) ) {
		...
		if( vSportType!= null && !vSportType.equalsIgnoreCase(Constants.LEADERBOARD)){
			...
			if( score_jArr != null && score_jArr.length() > 0 ) {
				...
				if ( team1_jObj.has( BASES_KEY ) ) {
					...
					if( baseArray.length() ==3 ) {
						for ( int k=0; k<3; k++) {
							base1 += baseArray.getBoolean(k) ? "1" :"0";
						}
					}
				}
				...
				json.parse(  new StringBuffer( team1_jObj.getJSONObject( TEAM_KEY ).toString() ) , Constants.TYPE_TEAM_JSON, true );
				 ...
				 if ( team1_jObj.has( SUMMARIES_KEY ) ) {
					 ...
					 for ( int j=0;j < summariesArray.length(); j++) {
						 dbUtil.setSummariesData(vHomeTeamId, vContestId, summariesArray.get(j).toString(), j+1 );
					 }
				 }
			}
			 ...
			 if( score_jArr != null && score_jArr.length() > 1 ) {
				 JSONObject team2_jObj = score_jArr.getJSONObject( 1 );
					...
					if ( team2_jObj.has( BASES_KEY ) ) {
						...
						if( baseArray.length() ==3 ) {
							for ( int k=0; k<3; k++) {
								base2 += baseArray.getBoolean(k) ? "1" :"0";
							}
						}
					}
					...
					if ( team2_jObj.has( TEAM_KEY ) ) {
					 ...
					 if ( team2_jObj.has( SUMMARIES_KEY ) ) {
						 ...
						 for ( int j=0;j < summariesArray.length(); j++) {
							 dbUtil.setSummariesData(vAwayTeamId, vContestId, summariesArray.get(j).toString(), j+1 );
						 }
					 }
			 }
			...
			for ( int k =0; k< score_jArr.length(); k++ ) {
				...
				JSONObject team = score_jArr.getJSONObject( k );	
			
				if( team.has(SUMMARY_KEY) )
					vSummary = team.getString( SUMMARY_KEY );
				if( team.has(POSITION_KEY) )
					iPosition = team.getInt( POSITION_KEY );
				if( team.has(POSITION_SUMMARY_KEY) )
					vPositionSummary = team.getString( POSITION_SUMMARY_KEY );
				...
				vTeamId = json.getTeamId();
				 ...
				 if ( team.has( SUMMARIES_KEY ) ) {
					 ...
					 for ( int j=0;j < summariesArray.length(); j++) {
						 dbUtil.setSummariesData(vTeamId, vContestId, summariesArray.get(j).toString(), j+1 );
					 }
				 }
		...
		if ( clock_jObj.has( MINUTES_KEY ) ) {
			...
			innings = clock_jObj.getInt( INNINGS_KEY );
		...
		String summary = clock_jObj.getString( SUMMARY_KEY );

		JSONObject contest_lobby_jObj = jsonObj.getJSONObject( LOBBY_KEY );

		String vContestLobbyUid = contest_lobby_jObj.getString( UID_KEY );
		String vContestLobbyUrl = contest_lobby_jObj.optString( SELF_KEY );

		String vContestLobbyHrefUrl = contest_lobby_jObj.optString( HREF_LINK_KEY );
		...
		String vEventsExtensionUrl = null,vEventsExtensionHrefUrl = "";
		...
		String vEventsUrl = null,vEventsId = null,vEventsHrefUrl = "";
		...
		dbUtil.setContestData ( vContestId, vContestUrl, scheduled_start_time, startTime, endTime, last_modified, shortTitle, title, iTotal1, vSummary1, vHomeTeamId, 
				...
				null,vCompetitionId,vCompetitionLiveId,competitionName,roundName,sportsName,annotation,
				...
				vRoundId ,vShareUrl, iHasLiveUpdates,vLastEventName, vShortMessage, vLongMessage,
				vSportType, active1, active2, associatedContestId,assoiciatedContestUrl,vEventsExtensionUrl,vEventsUrl,

				vEventsId , stadiumName ,vBackgroundImageUrl,assoiciatedContestHrefUrl,vContestHref,vEventsExtensionHrefUrl,vEventsHrefUrl,vShareHrefUrl);
