...
public class AssoiciatedContestsJsonUtil {
	...
	public AssoiciatedContestsJsonUtil ( JSONObject jsonObj, String vRoundContestId , boolean inTransaction,boolean leagues  ) {
	...
	public AssoiciatedContestsJsonUtil ( JSONObject jsonObj, String vRoundContestId, String vCompetitionLiveId, boolean inTransaction,boolean leagues ) {
...
//				new Util().getAssociatedContests(assoiciatedMatchUrl);
			...
			if( vSportType!= null && !vSportType.equalsIgnoreCase(Constants.LEADERBOARD)) {
				...
				if( score_jArr != null && score_jArr.length() > 0 ) {
					...
					json.parse(  new StringBuffer( team1_jObj.getJSONObject( TEAM_KEY ).toString() ) , Constants.TYPE_ASSOCIATED_TEAM_JSON, true );
					 ...
					 if ( team1_jObj.has( SUMMARIES_KEY ) ) {
							...
							for ( int j=0;j < summariesArray.length(); j++) {
								...
								if ( summaries_data != null && summaries_data.trim().length() > 0 ) {
									...
									dbUtil.setSummariesData(vHomeTeamId, vContestId, summaries_data, summaries_pos );
					 ...
					 try {
						 if ( (vSummary1 == null || vSummary1.trim().length() == 0) && team1_jObj.has( SUMMARIES_KEY ) ) {
							 JSONArray summariesArray = team1_jObj.getJSONArray( SUMMARIES_KEY );
				...
				if( score_jArr != null && score_jArr.length() > 1 ) {
					...
					if ( team2_jObj.has( TEAM_KEY ) ) {
						json.parse(  new StringBuffer( team2_jObj.getJSONObject( TEAM_KEY ).toString() ) , Constants.TYPE_ASSOCIATED_TEAM_JSON, true );
					}
					 ...
					 try {
						 if ( (vSummary2 == null || vSummary2.trim().length() == 0) && team2_jObj.has( SUMMARIES_KEY ) ) {
							 JSONArray summariesArray = team2_jObj.getJSONArray( SUMMARIES_KEY );
							 if( summariesArray.length() > 0 )
				...
				for ( int i =0; i< score_jArr.length(); i++ ) {
					...
					vSummary = team.optString( SUMMARY_KEY );
...
//					}
					...
					if ( team.has( TEAM_KEY ) ) {
						json.parse(  new StringBuffer( team.getJSONObject( TEAM_KEY ).toString() ) , Constants.TYPE_ASSOCIATED_TEAM_JSON, true );
					}
			...
			dbUtil.setContestDataForAssociatedContests ( vContestId, vContestUrl, scheduled_start_time, startTime, endTime, last_modified, shortTitle, title, iTotal1, vSummary1, vHomeTeamId, 
					...
					assoiciatedContestHrefUrl,vContestHrefUrl,vEventsExtensionHrefUrl,vEventsHrefUrl,vShareHrefUrl);
