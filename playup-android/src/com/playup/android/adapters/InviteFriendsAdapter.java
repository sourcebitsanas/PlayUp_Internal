...
import com.playup.android.fragment.InviteFriendFragment;
...
public class InviteFriendsAdapter extends BaseAdapter implements OnClickListener {
	...
	public InviteFriendsAdapter ( String vConversationId ,ListView listView, 
			InviteFriendFragment inviteFriendFragment,String vMainColor,String vMainTitleColor  , 
	...
	private void setValues () {
		...
		if ( inSearch ) {
			...
			if ( inviteFriendFragment.isDownloaded &&  ( ( alldata == null && recentdata == null ) || ( alldata != null && alldata.get( "vFriendId" ).size() == 0 ) && ( recentdata != null && recentdata.get( "vFriendId" ).size() == 0 ) ) ) {
	...
	public void setData ( String vConversationId ,ListView listView, InviteFriendFragment inviteFriendFragment,
	...
	public void setDummyData ( int count, InviteFriendFragment inviteFriendFragment  ) {
		hasZeroFriends = false;
		this.inviteFriendFragment = inviteFriendFragment;
		inSearch = false;
		isDummy = true;
	...
	public void setData ( Hashtable < String, List < String > > alldata,ListView listView, InviteFriendFragment inviteFriendFragment  ) {
		hasZeroFriends = false;
		this.inviteFriendFragment = inviteFriendFragment;
		inSearch = true;
		isDummy = false;
		...
		if ( isDummy ) {
			...
			try {
				if ( position == 0 ) {
					...
					// vh.avatarView.setVisibility( View.GONE );
					vh.userImage.setImageResource( R.drawable.blank_avatar1 );
		...
		try {
			...
			if ( ( position - HEADING  ) < totalRecentData  && totalRecentData > 0 ) {
				...
				if ( recentdata.get( "vSourceName" ) !=  null && recentdata.get( "vSourceName" ).get( position - 1 ) != null && recentdata.get( "vSourceName" ).get( position - 1 ).equalsIgnoreCase( "playup" ) ) {
					...
					if ( recentInviteUname.equalsIgnoreCase("null")){
				...
				/*String vCheckConversationId =  recentdata.get( "vConversationId" ).get( position -1 );
				if ( vCheckConversationId == null ) {
					vCheckConversationId = "";
				}
				if ( !vConversationId.equalsIgnoreCase(vCheckConversationId) ) {
					iStatus = 0;
				}
				 */
							...
							if ( !isGapLoading ) {
								...
								if (event.getAction() == MotionEvent.ACTION_DOWN) {
									...
									if (gapUid != null && gapUid.trim().length() > 0) {
										...
										InviteFriendFragment fragment = null;
										if ( PlayupLiveApplication.getFragmentManagerUtil().fragmentMap != null && PlayupLiveApplication.getFragmentManagerUtil().fragmentMap.containsKey( topFragmentName ) ) {
											fragment = ( InviteFriendFragment ) PlayupLiveApplication.getFragmentManagerUtil().fragmentMap.get( topFragmentName ); 
										}
	...
	public void onClick(View v) {
		...
		try {
			...
			//	Log.e("234","INSIE THE AWATAR VIEW==============>");
				...
				if ( vSelfUrl == null ) {
					//Toast.makeText( PlayUpActivity.context, R.string.profileDataNotAvailable, Toast.LENGTH_SHORT ).show();
					return;
				}
		...
		//		Log.e("234","INSIE THE status VIEW===========>>>"+status);
				if ( status == 0 ) {
					...
					setValues();
					notifyDataSetChanged();
					new Util().sendConversationInvitation ( vFriendId, vConversationId );
				}
			}
		} catch (NumberFormatException e) {
		//	Logs.show(e);
